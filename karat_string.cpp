...
vector<string> wrapLines(vector<string> &words, int maxLen) {
  ...
  while(i<n) {
    string t = words[i];
    if(output.size()) {
      if(output.size() + t.length() + 1 <= maxLen) {
        output+="-";
        output+=t; 
        i++; continue;
      } else {
...
int main() {
  vector<string> words1 = {"The","day","began","as","still","as","the","night","abruptly","lighted","with","brilliant","flame"};
  vector<string> words2 = {"Hello"};
  vector<string> words3 = {"Hello", "Hello"};
  vector<string> words4 = {"Well", "Hello", "world"};
  vector<string> words5 = {"Hello", "HelloWorld", "Hello", "Hello"};
  vector<string> words6 = {"a", "b", "c", "d"};
  vector<string> res = wrapLines(words1, 13);
    
res = wrapLines(words1, 13);    
p(res);


res = wrapLines(words1, 12);    
p(res);

res = wrapLines(words1, 20);    
p(res);

res = wrapLines(words2, 5);
p(res);

res = wrapLines(words2, 30);
p(res);

res = wrapLines(words3, 5);
p(res);

res = wrapLines(words4, 5);
p(res);

res = wrapLines(words5, 20);
p(res);

res = wrapLines(words6, 20);
p(res);

res = wrapLines(words6, 4);
p(res);
res = wrapLines(words6, 1);
p(res);

// p(wrapLines(words6, 20));
// p(wrapLines(words6, 4));
// p(wrapLines(words6, 1));
